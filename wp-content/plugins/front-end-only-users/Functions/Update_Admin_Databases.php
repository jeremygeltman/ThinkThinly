...
function Edit_EWD_FEUP_User( $User_ID, $User_Data_Array )
{
    global $wpdb;
    global $ewd_feup_user_table_name;
    //Brian3T added check for name or email changed
    $newUsername = $User_Data_Array['Username'];
    $oldUsername = $newUsername;
    $result = $wpdb->get_row("SELECT * FROM $ewd_feup_user_table_name WHERE `User_ID` = '$User_ID' ");
    ...
    if ( $newUsername != $oldUsername ) {
        session_start();
        $_SESSION['user_name_changed'] = true;
...
function Edit_EWD_FEUP_Field( $Field_ID, $Field_Name, $Field_Type, $Field_Description, $Field_Options, $Field_Show_In_Admin, $Field_Show_In_Front_End, $Field_Required )
{
    ...
    $update = __("Your settings have been updated.", 'EWD_FEUP');
...
function Edit_EWD_FEUP_User_Field( $Field_ID, $User_ID, $Field_Name, $Field_Value )
{
    ...
    $User_Field_ID = $wpdb->get_row($wpdb->prepare("SELECT User_Field_ID FROM $ewd_feup_user_fields_table_name WHERE Field_ID ='%d' AND User_ID='%d'",
                                                   $Field_ID, $User_ID));
...
function Update_EWD_FEUP_Email_Settings()
{
    ...
    if ( function_exists('mcrypt_decrypt') ) {
        $Encrypted_Admin_Password = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $Admin_Password,
                                                                 MCRYPT_MODE_CBC, md5(md5($key))));
    } else {
        $Encrypted_Admin_Password = $Admin_Password;
    }
